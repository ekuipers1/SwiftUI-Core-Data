...
public class Attribute: NSManagedObject, Identifiable {
    ...
    class func preview() -> Attribute {
        ...
        if attributes.count > 0 {
            ...
            let item = Item.createItem(name: "Item Preview", order: 999, itemdescription: "Basement")
            return Attribute.createAttributeFor(item: item, name: "Attribute Preview", order: 999, itemdescription: "Basement")
        }
    }
    #endif
    ...
    class func createAttributeFor(item: Item, name: String, order: Int?, itemdescription: String) -> Attribute {
        
        let attribute = Attribute.newAttribute()
        attribute.name = name
        attribute.order = Int32(order ?? 0)
        attribute.item = item
        attribute.itemdescription = itemdescription
        CoreData.stack.save()
        return attribute
    }
    
    public func update(name: String, order: String, itemdescription: String) {
